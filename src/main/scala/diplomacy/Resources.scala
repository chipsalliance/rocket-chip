...
trait DeviceInterrupts {
  ...
  def describeInterrupts(resources: ResourceBindings): Map[String, Seq[ResourceValue]] = {
    ...
    int.foreach { b =>
      require(b.device.isDefined, "Device ${devname} property 'int' is missing user device")
    }
...
trait DeviceRegName {
  ...
  def describeName(devname: String, resources: ResourceBindings): String = {
    ...
    if (reg.isEmpty) {
      ...
      val mainreg       = reg.find(x => DiplomacyUtils.regName(x._1) == Some("control")).getOrElse(reg.head)._2
...
trait BindingScope {
  ...
  private def makeTree(list: Seq[ExpandedValue]): Seq[ResourceValue] = {
    ...
    val keys = keys_p.groupBy(_.path.head).toList.map {
      case (key, seq) =>
        (key -> makeTree(seq.map { x =>
          x.copy(path = x.path.tail)
        }))
    }
