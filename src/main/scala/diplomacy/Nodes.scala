...
/** [[InwardNodeImp]] defines the types that describe the inward side of the [[BaseNode]].
  ...
  * @tparam EI The type of the Edge parameters (after diplomatic resolution) describing a connection on the inner side of the node.
  * @tparam BI The type of the Bundle used when connecting to the inner side of the node, corresponding to the real hardware interface that is emitted along the graph edge, generally parameterized by the EI type.
  */
trait InwardNodeImp[DI, UI, EI, BI <: Data]
{
  /** Create an inward-facing edge on this [[BaseNode]]
    *
    ...
    * @param p [[Parameters]] which can be accessed by this node as needed
  ...
  /** Defines how input parameters can be "mixed" or negotiated together.
    ...
    *
    * @param pu   upward flowing edge parameters
    * @param node an inward node to "mix" the upward flowing parameters into
    * @return altered version of the upward flowing inward parameters
    */
...
/** [[OutwardNodeImp]] defines the types that describe the outwards side of the [[BaseNode]].
  *
  * @tparam DO The type of the downwards flowing parameters generated by the outer side of the node
  * @tparam UO Tye type of the upwards flowing parameters received by the outer side of the node
  * @tparam EO The type of the edge parameters (after diplomatic resolution) describing a connection on the outer side of the node.
  * @tparam BO The type of the Bundle used when connecting to the outer side of the node, corresponding to the real hardware interface that is emitted along the graph edge, generally paramterized by the EO type.
  */
trait OutwardNodeImp[DO, UO, EO, BO <: Data]
{
  /** Create an outward-facing edge on this [[BaseNode]]
    *
    * @param pd         the downward flowing parameters out of the node along the edge
    * @param pu         the upward-flowing parameters going into the node along the edge
  ...
  /** Return the most-inward common node */
  def getI(pd: DO): Option[BaseNode] = None
}

/** The [[NodeImp]] combines an [[InwardNodeImp]] and an [[OutwardNodeImp]].
  *
  * This allows it to define whether it is a protocol-modifying (bridging)
  * sort of node, or whether it is an adapter type node that just modifies
  * the parameters within a protocol.
  *
...
abstract class BaseNode(implicit val valName: ValName)
{
  ...
  /** The index for this node in the containing [[LazyModule]]/[[LazyScope]]'s list of [[BaseNode]]s */
  ...
  /** @return [[BaseNode]] instance description, which can be overriden with more detailed information about each node. */
  def formatNode: String = ""

  /** @return metedata to visualize inward edges into this node. */
  def inputs:  Seq[(BaseNode, RenderedEdge)]
  /** @return metedata to visualize outward edges from this node. */
  def outputs: Seq[(BaseNode, RenderedEdge)]

  /** @return whether this node can handle [[BIND_FLEX]] type connections.
    */
  ...
  extends InwardNodeHandle[DI, UI, EI, BI] with OutwardNodeHandle[DO, UO, EO, BO]
{
  /** connects two full nodes handles => full node handle.
    ...
    * <---< := <---<  == <---<
    *
    * @param h source node also with sink handle.
  ...
  /** Connects a full node with an output => an output.
    ...
    *
    * @param h source node also without sink handle.
  ...
  extends BaseNode with NodeHandle[DI, UI, EI, BI, DO, UO, EO, BO] with InwardNode[DI, UI, BI] with OutwardNode[DO, UO, BO]
{
  ...
  /** Resolves the flex to be either source or sink and returns the offset where the "star" operators begin greedily taking up the remaining connections.
    *
    * @return The offset where star operators become greedy. A value >= 0 if it is sink cardinality, a negative value for source cardinality.
    */
  protected[diplomacy] lazy val flexOffset: Int = {
    ...
    // or when we get to a connection that is not a flex and can set the direction
    // for us.
  ...
  /** 
    ...
    * @return a positive number if source direction, negative otherwise.
    */
