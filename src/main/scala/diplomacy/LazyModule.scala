...
class LazyModuleCtx {
  ...
  */
  ...
  protected[diplomacy] var index = 0
}
/** While the [[freechips.rocketchip.diplomacy]] package allows fairly abstract parameter negotiation while constructing a DAG,
  ...
  */
abstract class LazyModule()(implicit val p: Parameters, val ctx: LazyModuleCtx) {
  ...
  protected[diplomacy] val parent: Option[LazyModule] = ctx.scope
...
object LazyModule {
  /** Wraps a [[LazyModule]], handling bookkeeping of scopes.
    ...
    */
  def apply[T <: LazyModule](bc: T)(implicit valName: ValName, sourceInfo: SourceInfo, ctx: LazyModuleCtx): T = {
    // Make sure the user puts [[LazyModule]] around modules in the correct order.
    require(ctx.scope.isDefined, s"LazyModule() applied to ${bc.name} twice ${sourceLine(sourceInfo)}. Ensure that descendant LazyModules are instantiated with the LazyModule() wrapper and that you did not call LazyModule() twice.")
    require(ctx.scope.get eq bc, s"LazyModule() applied to ${bc.name} before ${ctx.scope.get.name} ${sourceLine(sourceInfo)}")
...
sealed trait LazyModuleImpLike extends RawModule {
  implicit val ctx: LazyModuleCtx
  ...
  require(ctx.scope.isEmpty, s"${wrapper.name}.module was constructed before LazyModule() was run on ${ctx.scope.get.name}")
...
/** Actual description of a [[Module]] which can be instantiated by a call to [[LazyModule.module]].
  ...
  */
class LazyModuleImp(val wrapper: LazyModule)(implicit val ctx: LazyModuleCtx) extends MultiIOModule with LazyModuleImpLike {
...
/** Actual description of a [[RawModule]] which can be instantiated by a call to [[LazyModule.module]].
  ...
  * @param wrapper the [[LazyModule]] from which the `.module` call is being made.
  */
class LazyRawModuleImp(val wrapper: LazyModule)(implicit val ctx: LazyModuleCtx) extends RawModule with LazyModuleImpLike {
...
/** Used for a [[LazyModule]] which does not need to define any [[LazyModuleImp]] implementation.
  ...
  */
class SimpleLazyModule(implicit p: Parameters, ctx: LazyModuleCtx) extends LazyModule {
...
trait LazyScope {
  ...
  /** Evaluate `body` in the current [[LazyModuleCtx.scope]] */
  def apply[T](body: => T): T = {
    ...
    require(ctx.scope.isDefined, s"LazyScope $name tried to exit, but scope was empty!")
    require(ctx.scope.get eq this, s"LazyScope $name exited before LazyModule ${ctx.scope.get.name} was closed")
...
object LazyScope {
  /** Create a [[LazyScope]] with an implicit instance name.
    ...
    */
  def apply[T](body: => T)(implicit valName: ValName, p: Parameters, ctx: LazyModuleCtx): T = {
  ...
  /** Create a [[LazyScope]] with an explicitly defined instance name.
    ...
    * @param p         [[Parameters]] propagated to [[SimpleLazyModule]].
    */
  def apply[T](name: String)(body: => T)(implicit p: Parameters, ctx: LazyModuleCtx): T = {
  ...
  def apply[T](
    ...
    (body: => T)
    (implicit p: Parameters, ctx: LazyModuleCtx): T =
  {
  ...
  /** Create a [[LazyScope]] to temporarily group children for some reason, but tell Firrtl to inline it.
    ...
    * @param p         [[Parameters]] propagated to [[SimpleLazyModule]].
    */
  def inline[T](body: => T)(implicit p: Parameters, ctx: LazyModuleCtx): T = {
...
/** Used to inject code snippets to be evaluated in [[LazyModuleImp.instantiate]] in the current [[LazyModuleCtx.scope]].
  *
  ...
  */
object InModuleBody {
  def apply[T](body: => T)(implicit ctx: LazyModuleCtx): ModuleValue[T] = {
    require(ctx.scope.isDefined, s"InModuleBody invoked outside a LazyModule")
