...
object EspressoAddressDecoder
{
	...
	def apply(addr:UInt, ports: Ports): Seq[Bool] = {
		...
		decoder(addr, TruthTable(truthMap, dontCarePat)).asBools()
	}
...
class OrigialDecoder(ports: Seq[Seq[AddressSet]])(implicit XLen: Int) extends Decoder(ports.length) {
	...
	private val routeAddrs = ports.map {s => AddresSet.unify(s.map(_.widen(~routeMask)).distinct)}
	private val portFuncs = routeAddrs.map(s=>(addr: UInt) => s.map(_.contains(addr)).reduce(_ || _))

	ports:= portFuncs.map(_(addr))
}
...
class DecoderTop extends Module {
	...
	private val addressMap = Seq(
		Seq(AddressSet(BigInt("20000000",16), 0x1FFFFFFF),
		  AddressSet(BigInt("80000000",16), 0x1FFFFFFF)),
