...
class CLINTLogicalTreeNode(device: SimpleDevice, f: => OMRegisterMap) extends LogicalTreeNode(() => Some(device)) {
  ...
  def getOMCLINT(resourceBindings: ResourceBindings): Seq[OMComponent] = {
    val memRegions: Seq[OMMemoryRegion] =
      DiplomaticObjectModelAddressing.getOMMemoryRegions("CLINT", resourceBindings, Some(f))
...
class PLICLogicalTreeNode(device: => SimpleDevice, omRegMap: => OMRegisterMap, nPriorities: => Int, nInterrupts: => Int)
    extends LogicalTreeNode(() => Some(device)) {
  def getOMPLIC(resourceBindings: ResourceBindings): Seq[OMComponent] = {
    val memRegions: Seq[OMMemoryRegion] =
      DiplomaticObjectModelAddressing.getOMMemoryRegions("PLIC", resourceBindings, Some(omRegMap))
...
class BusMemoryLogicalTreeNode(
  ...
  busProtocolSpecification: Option[OMSpecification] = None
) extends LogicalTreeNode(() => Some(device)) {
  def getOMBusMemory(resourceBindings: ResourceBindings): Seq[OMComponent] = {
    val memRegions: Seq[OMMemoryRegion] =
      DiplomaticObjectModelAddressing.getOMMemoryRegions("OMMemory", resourceBindings, None)
...
class BusErrorLogicalTreeNode(device: => SimpleDevice, f: => OMRegisterMap)
    extends LogicalTreeNode(() => Some(device)) {
  def getOMBusError(resourceBindings: ResourceBindings): Seq[OMComponent] = {
    val Description(name, mapping) = device.describe(resourceBindings)

    val memRegions: Seq[OMMemoryRegion] =
      DiplomaticObjectModelAddressing.getOMMemoryRegions("BusError", resourceBindings, Some(f))
