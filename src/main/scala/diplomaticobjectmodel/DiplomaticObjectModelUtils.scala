...
import Chisel.{ log2Ceil, Data, Vec }
...
object DiplomaticObjectModelUtils {
  ...
  def getSuperClasses(klass: Class[_]): Seq[Class[_]] =
    if (klass == null)
      ...
      val parents =
        if (superKlass == null) interfaces
        else interfaces :+ superKlass

      val ancestors = for { parent <- parents; ancestor <- getSuperClasses(parent) } yield ancestor
...
class OMEnumSerializer
    extends CustomSerializer[OMEnum](format => {
...
object DiplomaticObjectModelAddressing {
  def getOMComponentHelper(
    ...
    fn: (ResourceBindings) => Seq[OMComponent]
  ): Seq[OMComponent] =
    fn(resourceBindings)
  ...
  def getOMMemoryRegions(
    name: String,
    resourceBindings: ResourceBindings,
    omRegMap: Option[OMRegisterMap] = None
  ): Seq[OMMemoryRegion] =
      ...
      case (x: String, seq: Seq[Binding]) if (DiplomacyUtils.regFilter(x) || DiplomacyUtils.rangeFilter(x)) =>
        seq.map {
          case Binding(device: Option[Device], value: ResourceValue) =>
            omMemoryRegion(name, DiplomacyUtils.regName(x).getOrElse(""), value, omRegMap)
        }
    }.flatten.toSeq

  def getOMPortMemoryRegions(
    name: String,
    resourceBindings: ResourceBindings,
    omRegMap: Option[OMRegisterMap] = None
  ): Seq[OMMemoryRegion] =
    resourceBindings.map.collect {
      case (x: String, seq: Seq[Binding]) if (DiplomacyUtils.regFilter(x) || DiplomacyUtils.rangeFilter(x)) =>
        seq.map {
          case Binding(device: Option[Device], value: ResourceValue) =>
            omMemoryRegion(name, "port memory region", value, omRegMap)
        }
    }.flatten.toSeq
