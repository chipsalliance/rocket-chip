...
case class TLToAXI4Node(stripBits: Int = 0)(implicit valName: ValName)
    extends MixedAdapterNode(TLImp, AXI4Imp)(dFn = { p =>
      ...
      val idSize = clients.map { c =>
        if (c.requestFifo) 1 else (c.sourceId.size >> stripBits)
      }
      ...
      AXI4MasterPortParameters(masters = masters, userBits = log2Ceil(p.endSourceId) + 4)
    }, uFn = { p =>
...
class TLToAXI4(val combinational: Boolean = true, val adapterName: Option[String] = None, val stripBits: Int = 0)(
...
object TLToAXI4 {
  def apply(combinational: Boolean = true, adapterName: Option[String] = None, stripBits: Int = 0)(
    implicit p: Parameters
  ) = {
