...
class TLAtomicAutomata(
  ...
  val node = TLAdapterNode(managerFn = {
    case mp =>
      mp.copy(managers = mp.managers.map {
        m =>
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        val violations = managersNeedingHelp.flatMap(_.findTreeViolation).map { node =>
          (node.name, node.inputs.map(_._1.name))
        }
        require(
          violations.isEmpty,
        ...
        managersNeedingHelp foreach { m =>
          require(m.fifoId.isDefined)
        }
