...
class TLBufferNode(a: BufferParams, b: BufferParams, c: BufferParams, d: BufferParams, e: BufferParams)(
  implicit valName: ValName
      ...
      clientFn = { p =>
        p.copy(minLatency = p.minLatency + b.latency + c.latency)
      },
      managerFn = { p =>
        p.copy(minLatency = p.minLatency + a.latency + d.latency)
      }
    ) {
...
object TLBuffer {
  ...
  def chain(depth: Int, name: Option[String] = None)(implicit p: Parameters): Seq[TLNode] = {
    ...
    name.foreach { n =>
      buffers.zipWithIndex.foreach { case (b, i) => b.suggestName(s"${n}_${i}") }
    }
    buffers.map(_.node)
  }
