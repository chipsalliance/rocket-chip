...
class AddressAdjuster(mask: BigInt, forceLocal: Seq[AddressSet] = Nil)(implicit p: Parameters) extends LazyModule {
  ...
  val node = TLNexusNode(
    ...
    managerFn = { mp =>
      ...
      local.managers.foreach { z =>
        checkMask(z)
      }
      remote.managers.foreach { z =>
        checkMask(z)
      }
      ...
      remote.managers.foreach {
        r =>
          require(
            errorDev.supportsAcquireT.contains(r.supportsAcquireT),
            s"Error device cannot cover ${r.name}'s AcquireT"
          )
          require(
            errorDev.supportsAcquireB.contains(r.supportsAcquireB),
            s"Error device cannot cover ${r.name}'s AcquireB"
          )
          require(
            errorDev.supportsArithmetic.contains(r.supportsArithmetic),
            s"Error device cannot cover ${r.name}'s Arithmetic"
          )
          require(
            errorDev.supportsLogical.contains(r.supportsLogical),
            s"Error device cannot cover ${r.name}'s Logical"
          )
          require(errorDev.supportsGet.contains(r.supportsGet), s"Error device cannot cover ${r.name}'s Get")
          require(
            errorDev.supportsPutFull.contains(r.supportsPutFull),
            s"Error device cannot cover ${r.name}'s PutFull"
          )
          require(
            errorDev.supportsPutPartial.contains(r.supportsPutPartial),
            s"Error device cannot cover ${r.name}'s PutPartial"
          )
      ...
      val newLocal = local.managers.map {
        l =>
          ...
          require(
            !any_in || !any_out,
            s"Address adjuster cannot have partially local devices, but: $forceLocal vs ${la}"
          )
          val fifoId = if (any_in) Some(ids.size) else Some(0)

          val container = remote.managers.find { r =>
            l.address.forall { la =>
              r.address.exists(_.contains(la))
            }
          }
          require(
            !container.isEmpty,
      ...
      val newRemote = ids.tail.zipWithIndex.flatMap {
        case (id, i) =>
          remote.managers.map { r =>
            r.copy(
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val a_support = VecInit(
      ...
      acquireOk,
      acquireOk
    )(parent.a.bits.opcode)

    val errorSet =
      localEdge.manager.managers.filter(_.nodePath.last.lazyModule.className == "TLError").head.address.head
