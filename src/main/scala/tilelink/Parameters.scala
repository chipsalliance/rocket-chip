...
import freechips.rocketchip.util.{ groupByIntoSeq, AsyncQueueParams, RationalDirection }
...
case class TLManagerParameters(
  ...
  address.foreach { a =>
    require(a.finite, "Address must be finite")
  }
...
case class TLClientPortParameters(clients: Seq[TLClientParameters], minLatency: Int = 0) // Only applies to B=>C
{
  ...
  def apply(managers: Seq[TLManagerParameters]): List[TLManagerParameters] = {
    ...
    managers.foreach { m =>
      ...
      map.get(k) match {
        ...
        case Some(n) => {
          map.update(k, m.copy(address = m.address ++ n.address, fifoId = None)) // Merging means it's not FIFO anymore!
        }
      }
    }
