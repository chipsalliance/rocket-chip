...
class TLFragmenter(
  ...
  val node = TLAdapterNode(
      ...
      c.copy(
        clients = Seq(
          TLClientParameters(
            ...
            userBits = {
              require(
                c.clients.forall(_.userBits.length == c.clients(0).userBits.length),
                s"Length of userBits sequences of all clients must be equal. ${c.clients.map(x => (x.name, x.userBits.length))}"
              )
              require(
                c.clients.forall(_.userBits.zip(c.clients(0).userBits).forall { case (a, b) => a.width == b.width }),
    ...
    managerFn = { m =>
      m.copy(managers = m.managers.map(mapManager))
    }
  )
