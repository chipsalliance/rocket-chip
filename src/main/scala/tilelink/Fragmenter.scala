    ...
    clientFn  = { c => c.v2copy(
      masters = Seq(TLMasterParameters.v2(
        ...
        emits       = TLMasterToSlaveTransferSizes(
          acquireT    = shrinkTransfer(c.masters.map(_.emits.acquireT)  .reduce(_ cover _)),
          acquireB    = shrinkTransfer(c.masters.map(_.emits.acquireB)  .reduce(_ cover _)),
          arithmetic  = shrinkTransfer(c.masters.map(_.emits.arithmetic).reduce(_ cover _)),
          logical     = shrinkTransfer(c.masters.map(_.emits.logical)   .reduce(_ cover _)),
          get         = shrinkTransfer(c.masters.map(_.emits.get)       .reduce(_ cover _)),
          putFull     = shrinkTransfer(c.masters.map(_.emits.putFull)   .reduce(_ cover _)),
          putPartial  = shrinkTransfer(c.masters.map(_.emits.putPartial).reduce(_ cover _)),
          hint        = shrinkTransfer(c.masters.map(_.emits.hint)      .reduce(_ cover _))
        )
      ))
    )},
