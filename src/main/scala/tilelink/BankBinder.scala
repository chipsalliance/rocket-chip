...
case class BankBinderNode(mask: BigInt)(implicit valName: ValName) extends TLCustomNode {
  ...
  def resolveStar(iKnown: Int, oKnown: Int, iStars: Int, oStars: Int): (Int, Int) = {
    ...
    require(
      ...
      s"${name} must have ${ports} inputs, but has ${iKnown} + ${iStar}*${iStars}$lazyModule.line}"
    )
    require(
      ports == oKnown + oStar * oStars,
      s"${name} must have ${ports} outputs, but has ${iKnown} + ${iStar}*${iStars}$lazyModule.line}"
    )
    (iStar, oStar)
  }
  ...
  def mapParamsD(n: Int, p: Seq[TLClientPortParameters]): Seq[TLClientPortParameters] =
    (p zip ids) map {
      case (cp, id) =>
        cp.copy(clients = cp.clients.map { c =>
          c.copy(
  ...
  def mapParamsU(n: Int, p: Seq[TLManagerPortParameters]): Seq[TLManagerPortParameters] =
    (p zip ids) map {
      case (mp, id) =>
        mp.copy(managers = mp.managers.flatMap { m =>
