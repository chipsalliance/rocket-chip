...
class TLFilter(
  ...
  val node = TLAdapterNode(
    clientFn = { cp =>
      cp.copy(clients = cp.clients.flatMap {
        c =>
          val out = cfilter(c)
          out.map {
    ...
    managerFn = { mp =>
      ...
      mp.copy(managers = managers, endSinkId = if (managers.exists(_.supportsAcquireB)) mp.endSinkId else 0)
    }
  )
...
object TLFilter {
  ...
  def mIdentity: ManagerFilter = { m =>
    Some(m)
  }
  // preserve client visibility
  def cIdentity: ClientFilter = { c =>
    Some(c)
  }
