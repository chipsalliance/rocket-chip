...
case class TLToAPBNode()(implicit valName: ValName)
    extends MixedAdapterNode(TLImp, APBImp)(dFn = {
      case TLClientPortParameters(clients, minLatency) =>
        val masters = clients.map {
          case c => APBMasterParameters(name = c.name, nodePath = c.nodePath, userBits = c.userBits)
        }
        APBMasterPortParameters(masters)
    }, uFn = {
      case APBSlavePortParameters(slaves, beatBytes) =>
