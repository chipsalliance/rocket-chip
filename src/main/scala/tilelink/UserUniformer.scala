...
class TLUserUniformer[T <: UserBits: ClassTag](default: UInt)(implicit p: Parameters) extends LazyModule {
  val node = TLAdapterNode(
    clientFn = { cp =>
      ...
      require(
        collected.forall(x => x.isEmpty || x.map(_.width).forall(y => y == longest.head.width)),
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        out.a.bits.user.zip(in.a.bits.user).foreach {
          case (ouser, iuser) =>
            val prev = edgeIn.getUserOrElse[T](in.a.bits, default)
            val mux = edgeOut.putUser(iuser, prev.map {
              x: UInt =>
                { (y: TLClientParameters) =>
                  x
                }
            })
