...
class TLWidthWidget(innerBeatBytes: Int)(implicit p: Parameters) extends LazyModule {
  val node = TLAdapterNode(clientFn = { case c => c }, managerFn = { case m => m.copy(beatBytes = innerBeatBytes) })
  ...
  lazy val module = new LazyModuleImp(this) {
    def merge[T <: TLDataChannel](edgeIn: TLEdge, in: DecoupledIO[T], edgeOut: TLEdge, out: DecoupledIO[T]) = {
      ...
      val enable = Seq.tabulate(ratio) { i =>
        !((count ^ UInt(i)) & limit).orR
      }
      ...
      edgeOut.data(out.bits) := (if (edgeIn.staticHasData(in.bits) == Some(false)) UInt(0)
                                 else helper(edgeIn.data(in.bits)))
      edgeOut.corrupt(out.bits) := corrupt_out

      (out.bits, in.bits) match {
        case (o: TLBundleA, i: TLBundleA) =>
          o.mask := edgeOut.mask(o.address, o.size) & Mux(hasData, helper(i.mask), ~UInt(0, width = outBytes))
