...
trait CanHaveMasterAXI4MemPort { this: BaseSubsystem =>
  ...
  val memAXI4Node = p(ExtMem).map {
    case MemoryPortParams(memPortParams, nMemoryChannels) =>
      val portName = "axi4"
...
trait CanHaveMasterAXI4MemPortModuleImp extends LazyModuleImp {
  ...
  (mem_axi4 zip outer.memAXI4Node) foreach {
    case (io, node) =>
...
trait CanHaveMasterAXI4MMIOPortModuleImp extends LazyModuleImp {
  ...
  def connectSimAXIMMIO() {
    (mmio_axi4 zip outer.mmioAXI4Node.in) foreach {
      case (io, (_, edge)) =>
      ...
      .map(
        params =>
          AXI4MasterPortParameters(
            masters = Seq(AXI4MasterParameters(name = portName.kebab, id = IdRange(0, 1 << params.idBits)))
          )
      )
      .toSeq
  )
      ...
      .map(
        params =>
          TLClientPortParameters(
            clients = Seq(TLClientParameters(name = portName.kebab, sourceId = IdRange(0, 1 << params.idBits)))
          )
      )
      .toSeq
  )
...
class SimAXIMem(edge: AXI4EdgeParameters, size: BigInt)(implicit p: Parameters) extends LazyModule {
  ...
  val srams = AddressSet.misaligned(0, size).map { aSet =>
    LazyModule(new AXI4RAM(aSet, beatBytes = edge.bundle.dataBits / 8))
  }
  val xbar = AXI4Xbar()
  srams.foreach { s =>
    s.node := AXI4Buffer() := AXI4Fragmenter() := xbar
  }
  xbar := node
