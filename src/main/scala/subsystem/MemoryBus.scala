...
case class BroadcastParams(nTrackers: Int = 4, bufferless: Boolean = false)
...
class MemoryBus(params: MemoryBusParams)(implicit p: Parameters)
  ...
  def inwardNode: TLInwardNode =
    if (params.replicatorMask == 0) xbar.node
    else {
      xbar.node :*= RegionReplicator(params.replicatorMask)
    }
