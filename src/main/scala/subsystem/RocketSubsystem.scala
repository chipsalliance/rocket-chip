...
trait HasRocketTiles extends HasTiles with CanHavePeripheryPLIC with CanHavePeripheryCLINT with HasPeripheryDebug {
  this: BaseSubsystem =>
  ...
  val rocketTiles = rocketTileParams.zip(crossings).map {
    case (tp, crossing) =>
...
trait HasRocketTilesModuleImp extends HasTilesModuleImp with HasPeripheryDebugModuleImp {
