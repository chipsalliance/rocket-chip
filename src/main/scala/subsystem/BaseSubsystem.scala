...
import freechips.rocketchip.diplomaticobjectmodel.logicaltree.{SubSystemLogicalTreeNode, _}
...
abstract class BaseSubsystem(implicit p: Parameters) extends BareSubsystem with HasLogicalTreeNode {
  ...
  LogicalModuleTree.add(LogicalModuleTree.rootLTN, logicalTreeNode)
}
