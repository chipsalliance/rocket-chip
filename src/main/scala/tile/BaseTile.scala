...
trait HasNonDiplomaticTileParameters {
  ...
  def tileProperties: PropertyMap = {
    ...
    val incoherent = if (!tileParams.core.useAtomicsOnlyForIO) Nil else Map("sifive,d-cache-incoherent" -> Nil)
    ...
    val dtlb = tileParams.dcache
      ...
      .map(d => Map("d-tlb-size" -> d.nTLBEntries.asProperty, "d-tlb-sets" -> 1.asProperty))
      .getOrElse(Nil)

    val itlb = tileParams.icache
      .filter(_ => tileParams.core.useVM)
      .map(i => Map("i-tlb-size" -> i.nTLBEntries.asProperty, "i-tlb-sets" -> 1.asProperty))
      .getOrElse(Nil)

    val mmu =
      if (!tileParams.core.useVM) Nil else Map("tlb-split" -> Nil, "mmu-type" -> s"riscv,sv$maxSVAddrBits".asProperty)
...
abstract class BaseTile private (val crossing: ClockCrossingType, q: Parameters)
  ...
  val bpwatchSourceNode = BundleBridgeSource(
    () => Vec(tileParams.core.nBreakpoints, new BPWatch(tileParams.core.retireWidth))
  )
