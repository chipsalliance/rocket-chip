...
class BusErrorUnit[T <: BusErrors](t: => T, params: BusErrorUnitParams, logicalTreeNode: LogicalTreeNode)(
  ...
  val node =
    TLRegisterNode(address = Seq(AddressSet(params.addr, params.size - 1)), device = device, beatBytes = p(XLen) / 8)
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val sources_enums = sources_and_desc.zipWithIndex.flatMap {
      case (s, i) =>
        s.map { e =>
          (BigInt(i) -> (e._2, e._3))
        }
    ...
    val (value, value_desc) = DescribedReg(
      UInt(width = sources.flatten.map(_.bits.getWidth).max),
      "value",
    ...
    val enable_desc = sources.zipWithIndex.map {
      case (s, i) =>
    ...
    val global_interrupt_desc = sources.zipWithIndex.map {
      case (s, i) =>
    ...
    val accrued_desc = sources.zipWithIndex.map {
      case (s, i) =>
    ...
    val local_interrupt_desc = sources.zipWithIndex.map {
      case (s, i) =>
