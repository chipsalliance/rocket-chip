...
trait HasFPUParameters {
  ...
  def consistent(x: UInt): Bool = {
    def helper(x: UInt, t: FType): Bool =
      if (typeTag(t) == 0) true.B
      else {
        val prevT      = prevType(t)
        val unswizzled = Cat(x(prevT.sig + prevT.exp - 1), x(t.sig - 1), x(prevT.sig + prevT.exp - 2, 0))
...
class FPToInt(implicit p: Parameters) extends FPUModule()(p) with ShouldBeRetimed {
  ...
  io.out.bits.store := (floatTypes.map(t => Fill(maxType.ieeeWidth / t.ieeeWidth, store(t.ieeeWidth - 1, 0))): Seq[
    UInt
  ])(tag)
...
class FPUFMAPipe(val latency: Int, val t: FType)(implicit p: Parameters) extends FPUModule()(p) with ShouldBeRetimed {
  require(latency > 0)
...
class FPU(cfg: FPUParams)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  @chiselName class FPUImpl { // entering gated-clock domain
    ...
    def useScoreboard(f: ((Pipe, Int)) => Bool) =
      pipes.zipWithIndex.filter(_._1.lat > 3).map(x => f(x)).fold(Bool(false))(_ || _)
    io.sboard_set := wb_reg_valid && !wb_cp_valid && Reg(
      next = useScoreboard(_._1.cond(mem_ctrl)) || mem_ctrl.div || mem_ctrl.sqrt
    )
