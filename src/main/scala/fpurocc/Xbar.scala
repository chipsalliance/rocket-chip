...
class NAMESPACEFanout()(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    if (node.edges.in.size >= 1) {
      require (node.edges.in.size == 1, "NAMESPACEFanout does not support multiple masters")
      require (node.edges.out.size > 0, "NAMESPACEFanout requires at least one slave")
