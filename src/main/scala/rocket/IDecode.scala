...
class IntCtrlSigs(implicit val p: Parameters) extends Bundle with HasRocketCoreParameters {
  if (usingCryptoNIST) require(alufn.SZ_ALU_FN >= ZKN.SZ_FN)
  if (usingCryptoSM) require(alufn.SZ_ALU_FN >= ZKS.SZ_FN)
  if (usingBitManipCrypto || usingBitManip) require(alufn.SZ_ALU_FN >= ZBK.SZ_FN)
