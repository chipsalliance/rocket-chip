...
class AMOALU(operandBits: Int)(implicit p: Parameters) extends Module {
  ...
  val io = new Bundle {
    ...
    val cmd: UInt = Input(UInt(M_SZ.W))
  ...
  val max: Bool = io.cmd === M_XA_MAX || io.cmd === M_XA_MAXU
  val min: Bool = io.cmd === M_XA_MIN || io.cmd === M_XA_MINU
  val add: Bool = io.cmd === M_XA_ADD
  val logic_and: Bool = io.cmd === M_XA_OR || io.cmd === M_XA_AND
  val logic_xor: Bool = io.cmd === M_XA_XOR || io.cmd === M_XA_OR
  ...
  io.out := wmask & out | (~wmask).asUInt & io.lhs
  io.out_unmasked := out
}
