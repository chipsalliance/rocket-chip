...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  withClock(gated_clock) { // entering gated-clock domain
    ...
    val (pte, invalid_paddr) = {
      ...
      when(tmp.r || tmp.w || tmp.x) {
        ...
        for (i <- 0 until pgLevels - 1)
          when(count <= i && tmp.ppn((pgLevels - 1 - i) * pgLevelBits - 1, (pgLevels - 2 - i) * pgLevelBits) =/= 0) {
            res.v := false
          }
      }
    ...
    val fragmented_superpage_ppn = {
      val choices =
        (pgLevels - 1 until 0 by -1).map(i => Cat(r_pte.ppn >> (pgLevelBits * i), r_req.addr(pgLevelBits * i - 1, 0)))
      choices(count)
    }
    ...
    val (l2_hit, l2_error, l2_pte, l2_tlb_ram) =
      if (coreParams.nL2TLBEntries == 0) (false.B, false.B, Wire(new PTE), None)
      else {
        ...
        when(io.dpath.sfence.valid) {
          valid :=
            Mux(
              ...
              valid & ~UIntToOH(io.dpath.sfence.bits.addr(idxBits + pgIdxBits - 1, pgIdxBits)),
    ...
    r_pte := OptimizationBarrier(
      Mux(
        ...
        Mux(
          ...
          l2_pte,
          Mux(
            state === s_fragment_superpage && !homogeneous,
            makePTE(fragmented_superpage_ppn, r_pte),
            Mux(
              state === s_req && pte_cache_hit,
              makePTE(pte_cache_data, l2_pte),
              Mux(arb.io.out.fire(), makePTE(io.dpath.ptbr.ppn, r_pte), r_pte)
            )
          )
        )
      )
