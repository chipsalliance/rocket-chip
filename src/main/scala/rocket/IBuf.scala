...
class IBuf(implicit p: Parameters) extends CoreModule {
  ...
  if (n > 0) {
    when(io.inst(0).ready) {
      ...
      if (n > 1) when(nReady > 0 && nReady < nBufValid) {
        val shiftedBuf =
          shiftInsnRight(buf.data(n * coreInstBits - 1, coreInstBits), (nReady - 1)(log2Ceil(n - 1) - 1, 0))
        buf.data := Cat(
  ...
  val icMask =
    (~UInt(0, fetchWidth * coreInstBits) << (nBufValid << log2Ceil(coreInstBits)))(fetchWidth * coreInstBits - 1, 0)
