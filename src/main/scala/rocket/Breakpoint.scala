...
class BreakpointUnit(n: Int)(implicit p: Parameters) extends CoreModule()(p) {
  ...
  (io.bpwatch zip io.bp).foldLeft((Bool(true), Bool(true), Bool(true))) {
    case ((ri, wi, xi), (bpw, bp)) =>
      ...
      when(end && r && ri) {
        io.xcpt_ld := (action === 0.U); io.debug_ld := (action === 1.U); bpw.valid(0) := true.B; bpw.dvalid(0) := true.B
      }
      when(end && w && wi) {
        io.xcpt_st := (action === 0.U); io.debug_st := (action === 1.U); bpw.valid(0) := true.B; bpw.dvalid(0) := true.B
      }
      when(end && x && xi) {
