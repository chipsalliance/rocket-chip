...
class DCacheErrors(implicit p: Parameters) extends L1HellaCacheBundle()(p) with CanHaveErrors {
  val correctable =
    (cacheParams.tagCode.canCorrect || cacheParams.dataCode.canCorrect).option(Valid(UInt(width = paddrBits)))
  val uncorrectable =
...
class DCacheDataArray(implicit p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  val data_arrays = Seq.tabulate(rowBytes / wordBytes) { i =>
    DescribedSRAM(
...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  @chiselName class DCacheModuleImpl { // entering gated-clock domain
    ...
    val s_ready :: s_voluntary_writeback :: s_probe_rep_dirty :: s_probe_rep_clean :: s_probe_retry :: s_probe_rep_miss :: s_voluntary_write_meta :: s_probe_write_meta :: Nil =
      Enum(UInt(), 8)
    ...
    val (s1_hit_way, s1_hit_state, s1_meta, s1_victim_meta) =
      if (usingDataScratchpad) {
        ...
        val s1_meta_hit_state = ClientMetadata.onReset.fromBits(
          s1_meta_uncorrected
            .map(r => Mux(r.tag === s1_tag && !s1_flush_valid, r.coh.asUInt, UInt(0)))
            .reduce(_ | _)
        )
    ...
    val s2_vaddr =
      Cat(RegEnable(s1_req.addr, s1_valid_not_nacked || s1_flush_valid) >> pgIdxBits, s2_req.addr(pgIdxBits - 1, 0))
    ...
    val s2_data = {
      ...
      val word_en = Mux(
        ...
        Mux(!s1_did_read, 0.U, UIntToOH(s1_req.addr.extract((rowBits / 8).log2 - 1, wordBytes.log2), wordsPerRow))
      )
    ...
    val s2_victim_way =
      Mux(s2_hit_valid, s2_hit_way, UIntToOH(RegEnable(s1_victim_way, s1_valid_not_nacked || s1_flush_valid)))
    val s2_victim_tag = Mux(
      s2_valid_data_error || s2_valid_flush_line,
      s2_req.addr(paddrBits - 1, tagLSB),
      RegEnable(s1_victim_meta.tag, s1_valid_not_nacked || s1_flush_valid)
    )
    val s2_victim_state =
      Mux(s2_hit_valid, s2_hit_state, RegEnable(s1_victim_meta.coh, s1_valid_not_nacked || s1_flush_valid))
    ...
    dataArb.io.in(0).bits.wordMask := UIntToOH(
      Mux(pstore2_valid, pstore2_addr, pstore1_addr).extract(rowOffBits - 1, offsetlsb)
    )
    ...
    tl_out_a.bits := Mux(
      ...
      Mux(!s2_write, get, Mux(!s2_read, put, atomics))
    )
    ...
    val releaseDataBeat = Cat(UInt(0), c_count) + Mux(
      releaseRejected,
      UInt(0),
      s1_release_data_valid + Cat(UInt(0), s2_release_data_valid)
    )
    ...
    dataArb.io
      .in(2)
      .bits
      .addr := (probeIdx(probe_bits) << blockOffBits) | (releaseDataBeat(log2Up(refillCycles) - 1, 0) << rowOffBits)
    ...
    if (usingRMW) {
      ...
      pstore1_storegen_data := (if (!usingDataScratchpad) amoalu.io.out
                                else {
    ...
    if (usingDataScratchpad) {
      ...
      val request_source =
        Seq(CoverBoolean(s2_isSlavePortAccess, Seq("from_TL")), CoverBoolean(!s2_isSlavePortAccess, Seq("from_CPU")))
      ...
      val data_error_dirty =
        Seq(CoverBoolean(!s2_victim_dirty, Seq("data_clean")), CoverBoolean(s2_victim_dirty, Seq("data_dirty")))
      val request_source = if (supports_flush) {
        Seq(CoverBoolean(!flushing, Seq("access")), CoverBoolean(flushing, Seq("during_flush")))
      } else {
