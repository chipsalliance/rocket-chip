...
class Rocket(tile: RocketTile)(implicit p: Parameters)
  ...
  @chiselName class RocketImpl { // entering gated-clock domain
    ...
    val perfEvents = new EventSets(
      Seq(
        new EventSet(
          (mask, hits) => Mux(mask(0), wb_xcpt, wb_valid && pipelineIDToWB((mask & hits).orR)),
          Seq(
            ("exception", () => false.B),
                  ...
                  Seq(
                    (
                      ...
                      () =>
                        ...
                        else id_ctrl.div && (id_ctrl.alu_fn & ALU.FN_DIV) =/= ALU.FN_DIV
                    ),
                    (
                      "div",
                      () =>
    ...
    val id_scie_decoder =
      if (!rocketParams.useSCIE) Wire(new SCIEDecoderInterface)
      else {
    ...
    val id_do_fence = Wire(
      init = id_rocc_busy && id_ctrl.fence ||
    ...
    val ex_op2 = MuxLookup(
      ex_ctrl.sel_alu2,
      SInt(0),
      Seq(A2_RS2 -> ex_rs(1).asSInt, A2_IMM -> ex_imm, A2_SIZE -> Mux(ex_reg_rvc, SInt(2), SInt(4)))
    )
    ...
    val ex_scie_unpipelined_wdata =
      if (!rocketParams.useSCIE) 0.U
      else {
    ...
    when(!ctrl_killd || csr.io.interrupt || ibuf.io.inst(0).bits.replay) {
      ...
      ex_reg_wphit := bpu.io.bpwatch.map { bpw =>
        bpw.ivalid(0)
      }
    }
    ...
    val mem_br_target = mem_reg_pc.asSInt +
      Mux(
        ...
        Mux(mem_ctrl.jal, ImmGen(IMM_UJ, mem_reg_inst), Mux(mem_reg_rvc, SInt(2), SInt(4)))
      )
    val mem_npc = (Mux(
    ...
    val (mem_ldst_xcpt, mem_ldst_cause) = checkExceptions(
      List((mem_debug_breakpoint, UInt(CSR.debugTriggerCause)), (mem_breakpoint, UInt(Causes.breakpoint)))
    )
    ...
    csr.io.pc := wb_reg_pc
    val tval_valid = wb_xcpt && wb_cause.isOneOf(
    ...
    val hazard_targets = Seq(
      (id_ctrl.rxs1 && id_raddr1 =/= UInt(0), id_raddr1),
    ...
    io.imem.btb_update.bits.cfiType :=
      Mux(
        ...
        CFIType.call,
        Mux(
          mem_ctrl.jalr && (mem_reg_inst(19, 15) & regAddrMask) === BitPat("b00?01"),
          CFIType.ret,
          Mux(mem_ctrl.jal || mem_ctrl.jalr, CFIType.jump, CFIType.branch)
        )
      )
    io.imem.btb_update.bits.target := io.imem.req.bits.pc
    io.imem.btb_update.bits.br_pc := (if (usingCompressed) mem_reg_pc + Mux(mem_reg_rvc, UInt(0), UInt(2))
                                      else mem_reg_pc)
    ...
    io.dmem.s1_data.data := (if (fLen == 0) mem_reg_rs2
                             else Mux(mem_ctrl.fp, Fill((xLen max fLen) / fLen, io.fpu.store_data), mem_reg_rs2))
