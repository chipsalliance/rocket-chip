...
class PMPReg(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  val addr: UInt = UInt((paddrBits - PMP.lgAlign).W)
  ...
  def readAddr: Bits = if (pmpGranularity.log2 == PMP.lgAlign) addr else {
...
class PMP(implicit p: Parameters) extends PMPReg {
  val mask: UInt = UInt(paddrBits.W)
  ...
  def aligned(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: PMP): Bool = if (lgMaxSize <= pmpGranularity.log2) true.B else {
    ...
    val straddlesLowerBound = ((x >> lgMaxSize) ^ (prev.comparand >> lgMaxSize)) === 0.U &&
      (prev.comparand(lgMaxSize-1, 0) & ~x(lgMaxSize-1, 0)) =/= 0.U
    val straddlesUpperBound = ((x >> lgMaxSize) ^ (comparand >> lgMaxSize)) === 0.U &&
      (comparand(lgMaxSize-1, 0) & (x(lgMaxSize-1, 0) | lsbMask)) =/= 0.U
...
class PMPChecker(lgMaxSize: Int)(implicit val p: Parameters) extends Module
  ...
  val io = IO(new Bundle {
    ...
    val pmp: Vec[PMP] = Input(Vec(nPMPs, new PMP))
  ...
  val res: PMP = (io.pmp zip (pmp0 +: io.pmp)).reverse.foldLeft(pmp0) { case (prev, (pmp, prevPMP)) =>
