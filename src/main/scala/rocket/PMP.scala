...
class PMPReg(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  def readAddr =
    if (pmpGranularity.log2 == PMP.lgAlign) addr
    else {
...
class PMP(implicit p: Parameters) extends PMPReg {
  ...
  def aligned(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: PMP): Bool =
    if (lgMaxSize <= pmpGranularity.log2) true.B
    else {
      val lsbMask = UIntToOH1(lgSize, lgMaxSize)
      val straddlesLowerBound = ((x >> lgMaxSize) ^ (prev.comparand >> lgMaxSize)) === 0 && (prev.comparand(
        lgMaxSize - 1,
        0
      ) & ~x(lgMaxSize - 1, 0)) =/= 0
      val straddlesUpperBound = ((x >> lgMaxSize) ^ (comparand >> lgMaxSize)) === 0 && (comparand(lgMaxSize - 1, 0) & (x(
        lgMaxSize - 1,
        0
      ) | lsbMask)) =/= 0
...
class PMPHomogeneityChecker(pmps: Seq[PMP])(implicit p: Parameters) {
  def apply(addr: UInt, pgLevel: UInt): Bool =
    ((true.B, 0.U.asTypeOf(new PMP)) /: pmps) {
      case ((h, prev), pmp) =>
        (h && pmp.homogeneous(addr, pgLevel, prev), pmp)
    }._1
}

class PMPChecker(lgMaxSize: Int)(implicit p: Parameters) extends CoreModule()(p) with HasCoreParameters {
  val io = new Bundle {
  ...
  val res = (pmp0 /: (io.pmp zip (pmp0 +: io.pmp)).reverse) {
    case (prev, (pmp, prevPMP)) =>
      ...
      for ((name, idx) <- Seq("no", "RO", "", "RW", "X", "RX", "", "RWX").zipWithIndex; if name.nonEmpty)
        cover(
          (Cat(pmp.cfg.x, pmp.cfg.w, pmp.cfg.r) === idx),
      ...
      for ((name, idx) <- Seq("", "TOR", "NA4", "NAPOT").zipWithIndex; if name.nonEmpty) {
        ...
        cover(
          pmp.cfg.l && hit && aligned && pmp.cfg.a === idx,
