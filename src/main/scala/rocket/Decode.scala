    ...
    Cat(
      (0 until default.getWidth.max(values.map(_.getWidth).max))
        .map({
          case (i: Int) =>
            val mint =
              termvalues.filter { case (k, t) => ((t.mask >> i) & 1) == 0 && ((t.value >> i) & 1) == 1 }.map(_._1)
            val maxt =
              termvalues.filter { case (k, t) => ((t.mask >> i) & 1) == 0 && ((t.value >> i) & 1) == 0 }.map(_._1)
...
object Simplify {
  ...
  def cheaper(a: List[Term], b: List[Term], bits: Int) = {
    ...
    def listLess(a: List[Term], b: List[Term]): Boolean =
      !b.isEmpty && (a.isEmpty || a.head < b.head || a.head == b.head && listLess(a.tail, b.tail))
  ...
  def getCover(implicants: Seq[Term], minterms: Seq[Term], bits: Int) =
    if (minterms.nonEmpty) {
      ...
      Seq[Term]()
  def stringify(s: Seq[Term], bits: Int) =
    s.map(
        t =>
          (0 until bits)
