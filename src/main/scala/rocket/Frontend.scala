...
class FrontendModule(outer: Frontend) extends LazyModuleImp(outer)
  ...
  if (usingBTB) {
    ...
    def scanInsns(idx: Int, prevValid: Bool, prevBits: UInt, prevTaken: Bool): Bool = {
      ...
      val rviBranch = rviBits(6,0) === Instructions.BEQ.value.asUInt.extract(6,0)
      val rviJump = rviBits(6,0) === Instructions.JAL.value.asUInt.extract(6,0)
      val rviJALR = rviBits(6,0) === Instructions.JALR.value.asUInt.extract(6,0)
