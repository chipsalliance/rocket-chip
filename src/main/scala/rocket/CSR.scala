...
class CSRFile(perfEventSets: EventSets = new EventSets(Seq()), customCSRs: Seq[CustomCSR] = Nil)(implicit p: Parameters)
  ...
  val (supported_interrupts, delegable_interrupts) = {
    ...
    val supported_high_interrupts =
      if (io.interrupts.buserror.nonEmpty) UInt(BigInt(1) << CSR.busErrorIntCause) else 0.U
  ...
  val s_interrupts = Mux(
    reg_mstatus.prv < PRV.S || (reg_mstatus.prv === PRV.S && reg_mstatus.sie),
  ...
  val fp_csrs =
    LinkedHashMap[Int, Bits](CSRs.fflags -> reg_fflags, CSRs.frm -> reg_frm, CSRs.fcsr -> Cat(reg_frm, reg_fflags))
  ...
  for (io_dec <- io.decode) {
    ...
    def decodeAny(m: LinkedHashMap[Int, Bits]): Bool =
      m.map { case (k: Int, _: Bits) => io_dec.csr === k }.reduce(_ || _)
    ...
    io_dec.fp_csr := Bool(usingFPU) && DecodeLogic(
      io_dec.csr,
      fp_csrs.keys.toList.map(_.U),
      (read_mapping -- fp_csrs.keys.toList).keys.toList.map(_.U)
    )
    ...
    (io_dec.csr.inRange(CSR.firstCtr, CSR.firstCtr + CSR.nCtr) || io_dec.csr
      .inRange(CSR.firstCtrH, CSR.firstCtrH + CSR.nCtr)) && !allow_counter ||
  ...
  val cause =
    Mux(insn_call, reg_mstatus.prv + Causes.user_ecall, Mux[UInt](insn_break, Causes.breakpoint, io.cause))
  ...
  val delegate = Bool(usingVM) && reg_mstatus.prv <= PRV.S && Mux(
    cause(xLen - 1),
  ...
  assert(
    PopCount(insn_ret :: insn_call :: insn_break :: io.exception :: Nil) <= 1,
  ...
  read_mapping.foreach({
    case (k, v) => {
      when(!k(11, 10).andR) { // Cover points for RW CSR registers
        cover(
          io.rw.cmd.isOneOf(CSR.W, CSR.S, CSR.C) && io.rw.addr === k,
  ...
  io.csrw_counter := Mux(
    coreParams.haveBasicCounters && csr_wen && (io.rw.addr.inRange(CSRs.mcycle, CSRs.mcycle + CSR.nCtr) || io.rw.addr
      .inRange(CSRs.mcycleh, CSRs.mcycleh + CSR.nCtr)),
  ...
  when(csr_wen) {
    ...
    if (usingFPU) {
      ...
      when(decoded_addr(CSRs.fcsr)) {
        set_fs_dirty := true; reg_fflags := wdata; reg_frm := wdata >> reg_fflags.getWidth
      }
    }
    if (usingDebug) {
