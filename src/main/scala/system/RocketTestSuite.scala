...
class BenchmarkTestSuite(makePrefix: String, val dir: String, val names: LinkedHashSet[String])
...
object DefaultTestSuites {
  ...
  val benchmarks = new BenchmarkTestSuite(
    ...
    LinkedHashSet("median", "multiply", "qsort", "towers", "vvadd", "dhrystone", "mt-matmul")
  )
  ...
  val emptyBmarks =
    new BenchmarkTestSuite("empty", "$(RISCV)/riscv64-unknown-elf/share/riscv-tests/benchmarks", LinkedHashSet.empty)

  val singleRegression = new RegressionTestSuite(LinkedHashSet("rv64ui-p-simple"))

  val mtBmarks = new BenchmarkTestSuite(
    "mt",
    "$(RISCV)/riscv64-unknown-elf/share/riscv-tests/mt",
    LinkedHashSet(
      ((0 to 4).map("vvadd" + _) ++
        List(
          "ad",
