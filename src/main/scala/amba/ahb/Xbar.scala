...
class AHBFanout()(implicit p: Parameters) extends LazyModule {
  val node = AHBFanoutNode(masterFn = { case Seq(m) => m }, slaveFn = { seq =>
    seq(0).copy(slaves = seq.flatMap(_.slaves))
  })
...
class AHBArbiter()(implicit p: Parameters) extends LazyModule {
  val node = AHBArbiterNode(masterFn = { case seq => seq(0).copy(masters = seq.flatMap(_.masters)) }, slaveFn = {
    case Seq(s) => s
  })
