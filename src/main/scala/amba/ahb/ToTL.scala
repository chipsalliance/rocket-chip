...
class AHBToTL()(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val a_burst_size = VecInit(burst_sizes.map(beats => log2Ceil(beats * beatBytes).U))(in.hburst)
      val a_burst_mask = VecInit(burst_sizes.map(beats => (beats * beatBytes - 1).U))(in.hburst)
