...
case class AHBSlavePortParameters(slaves: Seq[AHBSlaveParameters], beatBytes: Int) {
...
case class AHBMasterParameters(name: String, nodePath: Seq[BaseNode] = Seq(), userBits: Seq[UserBits] = Nil) {
  val userBitsWidth = userBits.map(_.width).sum
}

case class AHBMasterPortParameters(masters: Seq[AHBMasterParameters]) {
  val userBitsWidth = masters.map(_.userBitsWidth).max
}

case class AHBBundleParameters(addrBits: Int, dataBits: Int, userBits: Int) {
  require(dataBits >= 8)
  require(addrBits >= 1)
  ...
  def union(x: AHBBundleParameters) =
    AHBBundleParameters(max(addrBits, x.addrBits), max(dataBits, x.dataBits), userBits)
}
