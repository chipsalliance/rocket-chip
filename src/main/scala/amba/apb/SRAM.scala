  ...
  (implicit p: Parameters) extends DiplomaticSRAM(address, beatBytes, devName, logicalTreeNode)
{
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val bits: Int = 8
    val describedSRAM = makeSinglePortedByteWriteSeqMem(Vec(lanes, UInt(width = bits)),
    ...
    when (in.psel && !in.penable && in.pwrite && legal) {
      describedSRAM.mem.write(paddr, Vec.tabulate(beatBytes) { i => in.pwdata(8*(i+1)-1, 8*i) }, in.pstrb.asBools)
    }

    in.pready  := Bool(!fuzzReady) || LFSRNoiseMaker(1)(0)
    in.pslverr := RegEnable(!legal, !in.penable) || (Bool(fuzzError) && LFSRNoiseMaker(1)(0))
    in.prdata  := describedSRAM.mem.readAndHold(paddr, read).asUInt
  }
}
