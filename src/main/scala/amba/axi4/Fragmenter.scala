...
import freechips.rocketchip.util.{ leftOR, rightOR, OH1ToOH, UIntToOH1 }
...
class AXI4Fragmenter()(implicit p: Parameters) extends LazyModule {
  ...
  def mapSlave(s: AXI4SlaveParameters, beatBytes: Int) =
    s.copy(
  ...
  val node = AXI4AdapterNode(
    masterFn = { mp =>
      mp.copy(masters = mp.masters.map(m => mapMaster(m)), userBits = mp.userBits + 1)
    },
    slaveFn = { sp =>
    ...
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        def fragment(
          ...
          val sizes1 =
            (supportedSizes1 zip slave.slaves.map(_.address)).filter(_._1 >= 0).groupBy(_._1).mapValues(_.flatMap(_._2))
