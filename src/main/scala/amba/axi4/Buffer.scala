...
class AXI4Buffer(aw: BufferParams, w: BufferParams, b: BufferParams, ar: BufferParams, r: BufferParams)(
  ...
  val node = AXI4AdapterNode(masterFn = { p =>
    p
  }, slaveFn = { p =>
    p.copy(minLatency = p.minLatency + min(aw.latency, ar.latency) + min(r.latency, b.latency))
  })
...
object AXI4Buffer {
  ...
  def apply(aw: BufferParams, w: BufferParams, b: BufferParams, ar: BufferParams, r: BufferParams)(
    implicit p: Parameters
  ): AXI4Node = {
