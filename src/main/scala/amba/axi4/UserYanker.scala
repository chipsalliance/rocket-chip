  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        val rqueues = Seq.tabulate(edgeIn.master.endId) { i =>
          queue(i)
        }
        val wqueues = Seq.tabulate(edgeIn.master.endId) { i =>
          queue(i)
        }
        ...
        (rqueues zip (arsel zip rsel)) foreach {
          case (q, (ar, r)) =>
        ...
        (wqueues zip (awsel zip bsel)) foreach {
          case (q, (aw, b)) =>
