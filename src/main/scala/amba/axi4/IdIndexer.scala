  ...
  val node = AXI4AdapterNode(
    masterFn = { mp =>
      ...
      val masters = Array.tabulate(1 << idBits) { i =>
        AXI4MasterParameters(name = "", id = IdRange(i, i + 1), aligned = true, maxFlight = Some(0))
      }
      ...
      mp.masters.foreach { m =>
        for (i <- m.id.start until m.id.end) {
          ...
          val old = masters(j)
          names(j) += m.name
          masters(j) = old.copy(aligned = old.aligned && m.aligned, maxFlight = old.maxFlight.flatMap { o =>
            m.maxFlight.map { n =>
              o + n
            }
          })
      ...
      mp.copy(userBits = mp.userBits + max(0, log2Ceil(mp.endId) - idBits), masters = masters.zipWithIndex.map {
        case (m, i) => m.copy(name = names(i).toList.mkString(", "))
      })
    },
    slaveFn = { sp =>
      sp
