...
class AXI4Xbar(
      ...
      seq(0).copy(
        ...
        slaves = seq.flatMap { port =>
          require(
            ...
            s"Xbar data widths don't match: ${port.slaves.map(_.name)} has ${port.beatBytes}B vs ${seq(0).slaves
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val requestROI = io_out.map { o =>
      inputIdRanges.map { i =>
        i.contains(o.r.bits.id)
      }
    }
    val requestBOI = io_out.map { o =>
      inputIdRanges.map { i =>
        i.contains(o.b.bits.id)
      }
    }
    ...
    val requestWIO = awIn.map { q =>
      if (io_out.size > 1) q.io.deq.bits.asBools else Seq(Bool(true))
    }
      ...
      if (io_out.size > 1) {
        ...
        for (master <- edgesIn(i).master.masters) {
          ...
          for (id <- master.id.start until master.id.end) {
            ...
            awFIFOMap(id) := idTracker(awTag, awSel(id) && io_in(i).aw.fire(), bSel(id) && io_in(i).b.fire())
          }
        }
...
object AXI4Xbar {
  def apply(arbitrationPolicy: TLArbiter.Policy = TLArbiter.roundRobin, maxFlightPerId: Int = 7, awQueueDepth: Int = 2)(
    implicit p: Parameters
  ) = {
...
class AXI4XbarFuzzTest(name: String, txns: Int, nMasters: Int, nSlaves: Int)(implicit p: Parameters)
    extends LazyModule {
  val xbar           = AXI4Xbar()
