...
abstract class TLBusBypassBase(beatBytes: Int, deadlock: Boolean = false)(implicit p: Parameters) extends LazyModule {
  ...
  protected val bar = LazyModule(new TLBusBypassBar(dFn = { mp =>
    mp.copy(managers = mp.managers.map { m =>
      m.copy(mayDenyPut = m.mayDenyPut || !deadlock, mayDenyGet = m.mayDenyGet || !deadlock)
    })
  }))
...
class TLBypassNode(dFn: TLManagerPortParameters => TLManagerPortParameters)(implicit valName: ValName)
...
class TLBusBypassBar(dFn: TLManagerPortParameters => TLManagerPortParameters)(implicit p: Parameters)
    extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    require(
      ...
      s"BusBypass slave device widths mismatch (${edgeOut0.manager.managers
        .map(_.name)} has ${edgeOut0.manager.beatBytes}B vs ${edgeOut1.manager.managers.map(_.name)} has ${edgeOut1.manager.beatBytes}B)"
    )
