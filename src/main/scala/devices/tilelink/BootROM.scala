...
import freechips.rocketchip.subsystem.{BaseSubsystem, HierarchicalLocation, HasElements, TLBusWrapperLocation}
...
object BootROM {
  /** BootROM.attach not only instantiates a TLROM and attaches it to the tilelink interconnect
    ...
    */
  def attach(params: BootROMParams, subsystem: BaseSubsystem with HasElements, where: TLBusWrapperLocation)
