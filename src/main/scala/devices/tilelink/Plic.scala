...
class TLPLIC(params: PLICParams, beatBytes: Int)(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    require(
      nHarts > 0 && nHarts <= PLICConsts.maxHarts,
      s"Must be: nHarts=$nHarts > 0 && nHarts <= PLICConsts.maxHarts=${PLICConsts.maxHarts}"
    )

    // For now, use LevelGateways for all TL2 interrupts
    val gateways = interrupts.map {
      case i =>
    ...
    val priorityRegFields = priority.zipWithIndex.map {
      case (p, i) =>
    ...
    val enableRegFields = enables.zipWithIndex.map {
      case (e, i) =>
    ...
    val claiming = Seq
      .tabulate(nHarts) { i =>
        Mux(claimer(i), maxDevs(i), UInt(0))
      }
      .reduceLeft(_ | _)
    ...
    if (nDevices >= 2) {
      ...
      if (nPriorities > 0)
        ccover(
          maxDevs(0) > (UInt(1) << priority(0).getWidth) && maxDevs(0) <= Cat(UInt(1), threshold(0)),
          "THRESHOLD",
