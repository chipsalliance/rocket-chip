...
class TLDebugModuleInner(device: Device, getNComponents: () => Int, beatBytes: Int)(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this){
    ...
    val omRegMap = dmiNode.regmap(
      ...
      (DMI_AUTHDATA   << 2) -> (if (cfg.hasAuthentication) Seq(RWNotify(32, io.auth.get.dmAuthRdata, io.auth.get.dmAuthWdata, authRdEnMaybe, authWrEnMaybe,
                                 Some(RegFieldDesc("authdata", "authorization data exchange register", volatile=true)))) else Nil),
