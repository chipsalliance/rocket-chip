...
object SystemBusAccessModule {
  def apply(sb2tl: SBToTL, dmactive: Bool)(
      ...
      case (a, i) =>
        if (hasAddr(i)) {
          when(~dmactive) {
            ...
            a := Mux(
              SBADDRESSWrEn(i) && !SBCSRdData.sberror && !SBCSFieldsReg.sbbusy && !SBCSFieldsReg.sbbusyerror,
              SBADDRESSWrData(i),
              Mux(
    ...
    SBDATAFieldsReg.zipWithIndex.foreach {
      case (d, i) => d.zipWithIndex.foreach { case (d, j) => d.suggestName("SBDATA" + i + "BYTE" + j) }
    }
        ...
        if (hasData(i)) {
          ...
          for (j <- 0 to 3) {
            when(~dmactive) {
              ...
              d(j) := Mux(
                SBDATAWrEn(i) && !SBCSFieldsReg.sbbusy && !SBCSFieldsReg.sbbusyerror && !SBCSRdData.sberror,
    ...
    when(~dmactive) {
      ...
      SBCSFieldsReg.sbbusyerror := Mux(
        ...
        Mux(
          ...
          Mux((anyDataRdEn || anyDataWrEn) && sbBusy, true.B, SBCSFieldsReg.sbbusyerror)
        )
      ...
      for (i <- 0 until 3)
        sbErrorReg(i) := Mux(
          ...
          Mux(
            ...
            BadAddr.id.U(i), // Bad address accessed
            Mux(
              (tryWrEn || tryRdEn) && sbAlignmentError,
              AlgnError.id.U(i), // Address alignment error
              Mux(
                (tryWrEn || tryRdEn) && sbAccessError,
                BadAccess.id.U(i), // Access size error
                Mux(
...
class SBToTL(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val rdLegal_addr =
      edge.manager.supportsGetSafe(io.addrIn, io.sizeIn, Some(TransferSizes(1, cfg.maxSupportedSBAccess / 8)))
    val wrLegal_addr =
      edge.manager.supportsPutFullSafe(io.addrIn, io.sizeIn, Some(TransferSizes(1, cfg.maxSupportedSBAccess / 8)))
    ...
    when(sbState === SBReadRequest.id.U) { tl.a.bits := gbits }.otherwise { tl.a.bits := pfbits }
    ...
    counter := Mux((wrTxValid || rdTxValid) && txLast, 0.U, Mux((wrTxValid || rdTxValid), counter + 1.U, counter))
