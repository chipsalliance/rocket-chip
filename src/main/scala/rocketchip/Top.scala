...
/** Example Top with Hurricane Clocking */
class ExampleTopWithHurricaneClocks(p: Parameters) extends ExampleTop(p)
    with PeripheryHurricaneClocks {
  override lazy val module = Module(new ExampleTopWithHurricaneClocksModule(p, this, new ExampleTopWithHurricaneClocksBundle(p, _)))
}

class ExampleTopWithHurricaneClocksBundle(p: Parameters, c: Coreplex) extends ExampleTopBundle(p, c)
    with PeripheryHurricaneClocksBundle

class ExampleTopWithHurricaneClocksModule[+L <: ExampleTopWithHurricaneClocks, +B <: ExampleTopWithHurricaneClocksBundle](p: Parameters, l: L, b: Coreplex => B) extends ExampleTopModule(p, l, b)
    with PeripheryHurricaneClocksModule
