...
class AsyncResetReg(resetValue: Int = 0)
    extends BlackBox(Map("RESET_VALUE" -> IntParam(resetValue)))
...
object AsyncResetReg {
  ...
  def apply(updateData: UInt, resetData: BigInt, enable: Bool, name: String): UInt =
    apply(updateData, resetData, enable, Some(name))
