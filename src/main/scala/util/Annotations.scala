...
case class InterruptsPortAnnotation(target: Named, name: String, interruptIndexes: Seq[Int])
    extends SingleTargetAnnotation[Named] {
  def duplicate(n: Named) = this.copy(n)
}
...
case class RegFieldDescMappingAnnotation(target: ModuleName, regMappingSer: RegistersSer)
