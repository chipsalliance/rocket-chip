...
case class InterruptsPortAnnotation(target: Target, name: String, interruptIndexes: Seq[Int])
...
/** Record a case class that was used to parameterize this target. */
case class ParamsAnnotation(target: Target, paramsClassName: String, params: Map[String, Any])
    extends SingleTargetAnnotation[Target] {
  def duplicate(n: Target) = this.copy(n)
}
...
case class AddressMapAnnotation(target: Target, mapping: Seq[AddressMapEntry], label: String)
    extends SingleTargetAnnotation[Target] {
  def duplicate(n: Target) = this.copy(n)
...
case class TLToAXI4IdMapAnnotation(target: Target, mapping: Seq[TLToAXI4IdMapEntry])
    extends SingleTargetAnnotation[Target] {
  def duplicate(n: Target) = this.copy(n)
}
...
case class SlaveAddressMapChiselAnnotation(target: InstanceId, addresses: Seq[AddressSet], perms: ResourcePermissions)
    extends ChiselAnnotation {
  private val range = AddressRange.fromSets(addresses)
  def toFirrtl =
    AddressMapAnnotation(target = target.toNamed, mapping = range.map { r =>
      AddressMapEntry(r, perms, Nil)
    }, label = "slaves")
}
...
object Annotated {
  ...
  def interrupts(component: InstanceId, name: String, interrupts: Seq[Int]): Unit =
    annotate(new ChiselAnnotation {
      def toFirrtl: Annotation = InterruptsPortAnnotation(
        component.toNamed,
        name,
        interrupts
      )
  ...
  def port[T <: Data](
    ...
    names: Seq[String],
    width: Int,
    address: Seq[AddressSet] = Nil
  ): T = {
    annotate(new ChiselAnnotation {
      def toFirrtl = TopLevelPortAnnotation(data.toNamed, protocol, tags, names, width, address)
    })
    data
  }
}
...
trait ShouldBeRetimed { self: RawModule =>
  chisel3.experimental.annotate(new ChiselAnnotation {
    def toFirrtl: RetimeModuleAnnotation = RetimeModuleAnnotation(self.toNamed)
  })
}

case class RegFieldDescMappingAnnotation(target: Target, regMappingSer: RegistersSer)
    extends SingleTargetAnnotation[Target] {
...
//      }).filter(_.nonEmpty)}) )
  ...
  def anno(rawModule: RawModule, baseAddress: BigInt, mapping: RegField.Map*): Seq[RegField.Map] = {
