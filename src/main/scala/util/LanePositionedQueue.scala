...
class OnePortLanePositionedQueueModule[T <: Data](ecc: Code)(
  ...
  val ram_i        = Mux(write_row(1), VecInit(enq_buffer(2) ++ enq_buffer(3)), VecInit(enq_buffer(0) ++ enq_buffer(1)))
  val deq_row_half = deq_row >> 1
  val read_row =
    Mux(
      !isPow2(rows).B && deq_row_half === (rows / 2 - 1).U,
      1.U,
      Mux(!isPow2(rows).B && deq_row_half === (rows / 2 - 2).U, 0.U, deq_row_half + 2.U)
    )
  ...
  val deq_buf1 = VecInit.tabulate(4) { i =>
    deq_buffer((i + 1) % 4)
  }(deq_row(1, 0))
  for (l <- 0 until lanes) {
...
class PositionedQueueTest(queueFactory: LanePositionedQueue, lanes: Int, rows: Int, cycles: Int, timeout: Int = 500000)
  ...
  q.io.deq.bits.zipWithIndex.foreach {
    case (d, i) =>
