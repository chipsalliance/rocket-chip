...
class AsyncResetShiftReg(w: Int = 1, depth: Int = 1, init: Int = 0, name: String = "pipe")
...
class AsyncResetSynchronizerShiftReg(w: Int = 1, sync: Int = 3, init: Int = 0)
    extends AsyncResetShiftReg(w, depth = sync, init, name = "sync") {
...
class SynchronizerShiftReg(w: Int = 1, sync: Int = 3) extends AbstractPipelineReg(w) {
  ...
  (syncv.init zip syncv.tail).foreach {
    case (sink, source) =>
      sink := source
  }
  io.q := syncv.head
