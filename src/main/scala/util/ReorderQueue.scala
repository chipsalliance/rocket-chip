...
class ReorderEnqueueIO[T <: Data](dType: T, tagWidth: Int) extends DecoupledIO(new ReorderQueueWrite(dType, tagWidth)) {
...
class ReorderQueue[T <: Data](dType: T, tagWidth: Int, size: Option[Int] = None) extends Module {
  val io = new Bundle {
  ...
  if (tagSpaceSize > actualSize) {
    ...
    val roq_matches    = roq_tags.zip(roq_free).map { case (tag, free) => tag === io.deq.tag && !free }
