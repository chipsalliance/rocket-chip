...
class ResetCatchAndSync(sync: Int = 3) extends Module {
  ...
  withReset(post_psd_reset) {
    io.sync_reset := Mux(io.psd.test_mode, io.psd.test_mode_reset, ~AsyncResetSynchronizerShiftReg(Bool(true), sync))
  }
}

object ResetCatchAndSync {

  def apply(clk: Clock, rst: Bool, sync: Int = 3, name: Option[String] = None, psd: Option[PSDTestMode] = None): Bool =
