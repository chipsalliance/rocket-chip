...
class WithPowerQueueTests extends Config((site, here, up) => {
  case UnitTests => (q: Parameters) => {
    Seq(
      ...
      Module(new PositionedQueueTest(OnePortLanePositionedQueue(new IdentityCode), 4,  8, true,  false, 10000)),
      Module(new PositionedQueueTest(OnePortLanePositionedQueue(new IdentityCode), 1, 16, true,  false, 10000)),
