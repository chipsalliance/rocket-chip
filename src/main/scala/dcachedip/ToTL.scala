...
class HellaCacheToTL()(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val a_burst_ok =
        in.htrans === HellaCacheParameters.TRANS_NONSEQ && // only start burst on first HellaCache beat
        in.hsize  === UInt(log2Ceil(beatBytes))  && // not a narrow burst
        (in.haddr & a_burst_mask) === UInt(0)    && // address aligned to burst size
        in.hburst =/= HellaCacheParameters.BURST_INCR   && // we know the burst length a priori
      ...
      val a_access = in.htrans === HellaCacheParameters.TRANS_NONSEQ || in.htrans === HellaCacheParameters.TRANS_SEQ
      ...
      out.d.ready  := d_recv // backpressure AccessAckData arriving faster than HellaCache beats
      ...
      // To fix this would require a bus-wide register, and the HellaCache spec says this:
      ...
      // Therefore, we choose to accept this slight TL-HellaCache infidelity.
      ...
      // Make the failure persistent (and defer it to the last beat--otherwise HellaCache can cancel the burst!)
      d_fail :=
...
object HellaCacheToTL
{
  ...
  {
    val ahb2tl = LazyModule(new HellaCacheToTL)
    ahb2tl.node
  }
}
