...
import freechips.rocketchip.util.{ GenericParameterizedBundle, MuxSeq, ReduceOthers }
...
object RegMapper {
  ...
  def apply(bytes: Int, concurrency: Int, undefZero: Boolean, in: DecoupledIO[RegMapperInput], mapping: RegField.Map*)(
    ...
    def regIndexU(x: UInt) =
      if (maskBits == 0) UInt(0)
      else
    ...
    for (i <- 0 until flat.size) {
      ...
      val prepend = if (low == 0) {
        ...
        Cat(f_data, dataOut(reg) | UInt(0, width = low))
      }
    ...
    def mux(index: UInt, valid: Bool, select: Seq[Bool], guard: Seq[Bool], flow: Seq[Seq[(Bool, Bool)]]): Bool =
      MuxSeq(index, Bool(true), ((select zip guard) zip flow).map {
        case ((s, g), f) =>
          val out = Wire(Bool())
    ...
    out.bits.data := Mux(MuxSeq(oindex, Bool(true), oRightReg), MuxSeq(oindex, UInt(0), dataOut), UInt(0))
