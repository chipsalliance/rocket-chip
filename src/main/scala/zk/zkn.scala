...
class ZKNDE32Interface extends Bundle {
  val zkn_fn = Input(UInt(ZKN.FN_Len.W))
  val valid  = Input( Bool()    )
  val bs     = Input( UInt( 2.W))
...
class ZKNDE64Interface extends Bundle {
  val zkn_fn = Input(UInt(ZKN.FN_Len.W))
  val valid  = Input( Bool()    )
  val rcon   = Input( UInt( 4.W))
...
class ZKNImp(xLen:Int) extends Module {
  ...
  val t = aes_sbox(0, false.B, 0.U) //pseudo object call to include the verilog file of the poly16_mul module
}

class zknde32 extends BlackBox with HasBlackBoxInline {
  val io = IO(new ZKNDE32Interface)
  setInline("zknde32.v",
    s"""
       #module zknde32  (
       #input  [ 3:0] zkn_fn,
       ...
       #
       #// SBOX instances
       #aes_sbox  #(.IMP(1)) i_aes_sbox( .fw (~dec), .in(sel_byte), .fx(sbox_out) );
       #
       #endmodule
...
class zknde64 extends BlackBox with HasBlackBoxInline {
  val io = IO(new ZKNDE64Interface)
  setInline("zknde64.v",
    s"""
       #module zknde64  (
       #input  [ 3:0] zkn_fn,
       ...
       #wire [ 7:0] sb_out [7:0];
       #
       #wire [63:0] sbox     = { sb_out[7], sb_out[6], sb_out[5], sb_out[4], sb_out[3], sb_out[2], sb_out[1], sb_out[0] };
       #
       #//
       ...
       #
       #wire [31:0] ks1_sbout   = sbox[31:0] ^ {24'b0, rconst};
       #
       #// If just doing sub-bytes, sbox inputs direct from rs1.
       #assign      sb_in[0]= (ks1_sel) ? ks1_sb0: (dec_sel) ? `BY(shiftrows_dec, 0) : `BY(shiftrows_enc, 0);
       #assign      sb_in[1]= (ks1_sel) ? ks1_sb1: (dec_sel) ? `BY(shiftrows_dec, 1) : `BY(shiftrows_enc, 1);
       #assign      sb_in[2]= (ks1_sel) ? ks1_sb2: (dec_sel) ? `BY(shiftrows_dec, 2) : `BY(shiftrows_enc, 2);
       #assign      sb_in[3]= (ks1_sel) ? ks1_sb3: (dec_sel) ? `BY(shiftrows_dec, 3) : `BY(shiftrows_enc, 3);
       #assign      sb_in[4]=                      (dec_sel) ? `BY(shiftrows_dec, 4) : `BY(shiftrows_enc, 4);
       #assign      sb_in[5]=                      (dec_sel) ? `BY(shiftrows_dec, 5) : `BY(shiftrows_enc, 5);
       #assign      sb_in[6]=                      (dec_sel) ? `BY(shiftrows_dec, 6) : `BY(shiftrows_enc, 6);
       #assign      sb_in[7]=                      (dec_sel) ? `BY(shiftrows_dec, 7) : `BY(shiftrows_enc, 7);
       #
       #
       #// MixColumns
       #// ------------------------------------------------------------
       ...
       #wire [31:0] mix_enc_i0  = sbox[31: 0];
       #wire [31:0] mix_enc_i1  = sbox[63:32];
       #
       #// Inverse MixColumns inputs.
       #wire [31:0] mix_dec_i0  = im_sel ? rs1[31: 0] : sbox[31: 0];
       #wire [31:0] mix_dec_i1  = im_sel ? rs1[63:32] : sbox[63:32];
       #
       ...
       #
       #wire [63:0] result_enc  = (encsm_sel || decsm_sel) ? {mix_enc_o1, mix_enc_o0} : sbox  ;
       #wire [63:0] result_dec  = (encsm_sel || decsm_sel) ? {mix_dec_o1, mix_dec_o0} : sbox  ;
       #
       ...
       #        aes_sbox #(.IMP(1)) i_sbox ( .fw(enc_sel||ks1_sel), .in(sb_in [i]), .fx(sb_out[i]) );
       #    end
       #endgenerate
       #
       #//
...
class aes_sbox(Imp: Int) extends BlackBox(Map("IMP" -> Imp)) with HasBlackBoxInline {
  val io = IO(new Bundle {
    val fw = Input(Bool())
    val in = Input(UInt(8.W))
    val fx = Output(UInt(8.W))
  })
  setInline("aes_sbox.v",
    s"""
       #// AES SBox
       #module aes_sbox #(parameter IMP = 1)(  // 0: Null Sbox; otherwise: Do Sbox
       #input        fw,
       #input  [7:0] in,
       #output [7:0] fx
       #
       #);
       #
       #wire [20:0] fwd_top, inv_top, top_box;
       #wire [17:0] mid;
       #generate
       #if (IMP!=0) begin: do_Sbox
       #
       #aes_sbox_top     fwdtop_ins( .y(fwd_top), .x(in) );
       #aes_inv_sbox_top invtop_ins( .y(inv_top), .x(in) );
       #
       #assign top_box = (fw)? fwd_top : inv_top;
       #
       #aes_sbox_inv_mid mid_ins ( .y(mid), .x(top_box) );
       #
       #wire [7:0] fwd_out, inv_out;
       #aes_sbox_out     fwd_out_ins( .y(fwd_out), .x(mid) );
       #aes_inv_sbox_out inv_out_ins( .y(inv_out), .x(mid) );
       #
       #assign      fx = (fw)? fwd_out : inv_out;
       #end
       #else assign fx = 8'h00;
       #endgenerate
       #
       #endmodule
       #
...
object aes_sbox {
  def apply(Imp: Int, fw: Bool, in: UInt) = {
    val m = Module(new aes_sbox(Imp))
    m.io.fw := fw
    m.io.in := in
    m.io.fx
  }
