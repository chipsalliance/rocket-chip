  ...
  lazy val emulator: Path = {
    ...
    os.write(
      ...
      s"""cmake_minimum_required(VERSION 3.20)
         ...
         |set(CMAKE_CXX_FLAGS "$${CMAKE_CXX_FLAGS} -DVERILATOR -DTEST_HARNESS=VTestHarness -std=c++11 -include ${resource("resources/csrc/verilator.h")} -include ${outputDirectory / "TestHarness.plusArgs"} -include VTestHarness.h")
